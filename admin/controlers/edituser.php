<?php
    $xtpa = new XTemplate('views/edituser.html');
        $condition = '1=1';

        $id = $_GET['id'];
        $edit = $db->fetchAll('users',"id=$id");
        $name = $edit[0]['name'];
        $email = $edit[0]['email'];
        ...
        if($do_save == 1){
            $pl = "name = '$name', email = '$email', phone = '$phone', address = '$address', password = '$password'";
